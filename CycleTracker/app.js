...
// create constants for the form and the foorm controls
...
newPeriodFormEl.addEventListener("submit", (event) => {
  ...
  // since everthing is client-side.
...
function checkDatesInvalid(startDate, endDate) {
  ...
  if (!startDate || !endDate || startDate > endDate) {
    ...
    // instance, for now, we clear the dates if either
    // or both are invalid.
    newPeriodFormEl.reset();
...
function getAllStoredPeriods() {
  ...
  // if no period were stored, default to an empty array
...
function formatDate(dateString) {
  // convert the dat string to a Date object
  const date = new Date(dateString);

  // format the date into a local-specific string.
